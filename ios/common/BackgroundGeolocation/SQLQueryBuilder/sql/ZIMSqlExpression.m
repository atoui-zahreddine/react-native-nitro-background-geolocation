...
+ (NSString *) prepareOperator: (NSString *)sqlOperator ofType: (NSString *)type {
	if ([[type uppercaseString] isEqualToString: @"SET"] && ![sqlOperator matchesRegex: @"^(except|intersect|(union( all)?))$" options: NSRegularExpressionCaseInsensitive]) {
		@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Invalid set operator token provided." userInfo: nil];
	}
	return [sqlOperator uppercaseString];
}
