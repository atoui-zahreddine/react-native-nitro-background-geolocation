...
- (void) joinOn: (id)column1 operator: (NSString *)sqlOperator column: (id)column2 {
	[self joinOn: column1 operator: sqlOperator column: column2 connector: ZIMSqlConnectorAnd];
}

- (void) joinOn: (id)column1 operator: (NSString *)sqlOperator column: (id)column2 connector: (NSString *)connector {
	...
	if (length > 0) {
		...
		[joinCondition addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@", [ZIMSqlExpression prepareIdentifier: column1], [sqlOperator uppercaseString], [ZIMSqlExpression prepareIdentifier: column2]]]];
	}
	else {
...
- (void) joinOn: (id)column operator: (NSString *)sqlOperator value: (id)value {
	[self joinOn: column operator: sqlOperator value: value connector: ZIMSqlConnectorAnd];
}

- (void) joinOn: (id)column operator: (NSString *)sqlOperator value: (id)value connector: (NSString *)connector {
	NSUInteger length = [_join count];
	if (length > 0) {
		NSUInteger index = length - 1;
		NSMutableArray *joinCondition = (NSMutableArray *)[[_join objectAtIndex: index] objectAtIndex: 2];
		...
		joinCondition = (NSMutableArray *)[[_join objectAtIndex: index] objectAtIndex: 1];
		sqlOperator = [sqlOperator uppercaseString];
		if ([sqlOperator isEqualToString: ZIMSqlOperatorBetween] || [sqlOperator isEqualToString: ZIMSqlOperatorNotBetween]) {
			...
			[joinCondition addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@ AND %@", [ZIMSqlExpression prepareIdentifier: column], sqlOperator, [ZIMSqlExpression prepareValue: [(NSArray *)value objectAtIndex: 0]], [ZIMSqlExpression prepareValue: [(NSArray *)value objectAtIndex: 1]]]]];
		}
		else {
			if (([sqlOperator isEqualToString: ZIMSqlOperatorIn] || [sqlOperator isEqualToString: ZIMSqlOperatorNotIn]) && ![value isKindOfClass: [NSArray class]]) {
				@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Operator requires the value to be declared as an array." userInfo: nil];
			}
			else if ([value isKindOfClass: [NSNull class]]) {
				if ([sqlOperator isEqualToString: ZIMSqlOperatorEqualTo]) {
					sqlOperator = ZIMSqlOperatorIs;
				}
				else if ([sqlOperator isEqualToString: ZIMSqlOperatorNotEqualTo] || [sqlOperator isEqualToString: @"!="]) {
					sqlOperator = ZIMSqlOperatorIsNot;
				}
			}
			[joinCondition addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@", [ZIMSqlExpression prepareIdentifier: column], sqlOperator, [ZIMSqlExpression prepareValue: value]]]];
		}
	}
	else {
		@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Must declare a JOIN clause before declaring a constraint." userInfo: nil];
...
- (void) where: (id)column1 operator: (NSString *)sqlOperator column: (id)column2 {
	[self where: column1 operator: sqlOperator column: column2 connector: ZIMSqlConnectorAnd];
}

- (void) where: (id)column1 operator: (NSString *)sqlOperator column: (id)column2 connector: (NSString *)connector {
	[_where addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@", [ZIMSqlExpression prepareIdentifier: column1], [sqlOperator uppercaseString], [ZIMSqlExpression prepareIdentifier: column2]]]];
}

- (void) where: (id)column operator: (NSString *)sqlOperator value: (id)value {
	[self where: column operator: sqlOperator value: value connector: ZIMSqlConnectorAnd];
}

- (void) where: (id)column operator: (NSString *)sqlOperator value: (id)value connector: (NSString *)connector {
	sqlOperator = [sqlOperator uppercaseString];
	if ([sqlOperator isEqualToString: ZIMSqlOperatorBetween] || [sqlOperator isEqualToString: ZIMSqlOperatorNotBetween]) {
		if (![value isKindOfClass: [NSArray class]]) {
			@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Operator requires the value to be declared as an array." userInfo: nil];
		}
		[_where addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@ AND %@", [ZIMSqlExpression prepareIdentifier: column], sqlOperator, [ZIMSqlExpression prepareValue: [(NSArray *)value objectAtIndex: 0]], [ZIMSqlExpression prepareValue: [(NSArray *)value objectAtIndex: 1]]]]];
	}
	else {
		if (([sqlOperator isEqualToString: ZIMSqlOperatorIn] || [sqlOperator isEqualToString: ZIMSqlOperatorNotIn]) && ![value isKindOfClass: [NSArray class]]) {
			@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Operator requires the value to be declared as an array." userInfo: nil];
		...
		[_where addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@", [ZIMSqlExpression prepareIdentifier: column], sqlOperator, [ZIMSqlExpression prepareValue: value]]]];
	}
}
...
- (void) groupByHaving: (id)column1 operator: (NSString *)sqlOperator column: (id)column2 {
	[self groupByHaving: column1 operator: sqlOperator column: column2 connector: ZIMSqlConnectorAnd];
}

- (void) groupByHaving: (id)column1 operator: (NSString *)sqlOperator column: (id)column2 connector: (NSString *)connector {
	if ([_groupBy count] > 0) {
		[_having addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@", [ZIMSqlExpression prepareIdentifier: column1], [sqlOperator uppercaseString], [ZIMSqlExpression prepareIdentifier: column2]]]];
	}
	else {
		@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Must declare a GROUP BY clause before declaring a constraint." userInfo: nil];
	}
...
- (void) groupByHaving: (id)column operator: (NSString *)sqlOperator value: (id)value {
	[self groupByHaving: column operator: sqlOperator value: value connector: ZIMSqlConnectorAnd];
}

- (void) groupByHaving: (id)column operator: (NSString *)sqlOperator value: (id)value connector: (NSString *)connector {
	if ([_groupBy count] > 0) {
		sqlOperator = [sqlOperator uppercaseString];
		if ([sqlOperator isEqualToString: ZIMSqlOperatorBetween] || [sqlOperator isEqualToString: ZIMSqlOperatorNotBetween]) {
			if (![value isKindOfClass: [NSArray class]]) {
			...
			[_having addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@ AND %@", [ZIMSqlExpression prepareIdentifier: column], sqlOperator, [ZIMSqlExpression prepareValue: [(NSArray *)value objectAtIndex: 0]], [ZIMSqlExpression prepareValue: [(NSArray *)value objectAtIndex: 1]]]]];
		}
		else {
			if (([sqlOperator isEqualToString: ZIMSqlOperatorIn] || [sqlOperator isEqualToString: ZIMSqlOperatorNotIn]) && ![value isKindOfClass: [NSArray class]]) {
				@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Operator requires the value to be declared as an array." userInfo: nil];
			...
			[_having addObject: @[[ZIMSqlExpression prepareConnector: connector], [NSString stringWithFormat: @"%@ %@ %@", [ZIMSqlExpression prepareIdentifier: column], sqlOperator, [ZIMSqlExpression prepareValue: value]]]];
		}
	}
	else {
		@throw [NSException exceptionWithName: @"ZIMSqlException" reason: @"Must declare a GROUP BY clause before declaring a constraint." userInfo: nil];
...
- (void) combine: (NSString *)statement operator: (NSString *)sqlOperator {
	...
	[_combine addObject: [NSString stringWithFormat: @"%@ %@", [ZIMSqlExpression prepareOperator: sqlOperator ofType: @"SET"], statement]];
}
